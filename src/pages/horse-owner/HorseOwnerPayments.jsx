...
export function HorseOwnerPayments() {
  ...
  const totalPrize = payments
    .filter((p) => p.status === "Đã nhận")
    .reduce((s, p) => s + p.amount, 0);
          ...
          <div className="p-5 space-y-3">
            {payments
              ...
              .map((p) => (
                <div
                  key={p.id}
                  className="flex items-center justify-between gap-4 p-4 bg-[#D4A017]/5 border border-[#D4A017]/20 rounded-xl"
                >
                  <div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <PrimaryButton
                      onClick={() =>
                        toast.success(
                          `Đã thanh toán ${fmt(p.amount)} · ${p.tournament}`,
                        )
                      }
                    >
                      Thanh toán ngay
              ...
              {payments.map((p) => (
                ...
                >
                  ...
                  <td className="px-5 py-4 text-right">
                    <span
                      className={`text-sm font-bold ${p.type === "Tiền thưởng" ? "text-emerald-300" : "text-white"}`}
                    >
                      {p.type === "Tiền thưởng" ? "+" : "-"}
                      {fmt(p.amount)}
                    </span>
