...
import { isRaceToday, getRefereeRaceStatusTone } from '@/utils/refereeRaceUtils';
  ...
  useEffect(() => {
    ...
    async function loadDashboardStats() {
      try {
        ...
        const [checkedIn, pending] = await Promise.all([
          refereeService.getCheckedInCount(),
          refereeService.getPendingCheckInCount(),
        ]);
        if (!cancelled) {
          setCheckedInCount(checkedIn);
          setPendingCheckInCount(pending);
        }
      } catch {
        if (!cancelled) {
          setCheckedInCount(0);
          setPendingCheckInCount(0);
        }
      } finally {
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <StatCard
          label="Đã check-in"
          value={statsLoading ? '...' : String(checkedInCount)}
          icon={CheckCircle2}
          tone="green"
        />
        <StatCard
          label="Chờ check-in"
          value={statsLoading ? '...' : String(pendingCheckInCount)}
          icon={Clock}
          tone="blue"
        />
